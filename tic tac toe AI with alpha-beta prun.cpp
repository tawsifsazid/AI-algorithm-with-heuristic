...
int cases,ct = 10,r,c,y,cnt = 1 , prun = 0;
char A[40][40];
struct Node{
    int row,col;
};
...
int min_max(int turn,int alpha,int beta){
    int check = evaluate();
        ...
        for(i=1;i<=3;i++){
            for(j=1;j<=3;j++){
                if(A[i][j] == '.'){
                    ...
                    //int aa = min_max(!turn);
                    init = max(init,min_max(!turn, alpha, beta));
                    if(init >= alpha){
                        alpha = init;
                    }
        ...
        for(i=1;i<=3;i++){
            for(j=1;j<=3;j++){
                if(A[i][j] == '.'){
                    ...
                    init = min(init,min_max(!turn , alpha, beta));
                    if(init <= beta){
                       beta = init;
                    }
    ...
    for(i = 1; i<=3; i++){
        for(j=1;j<=3;j++){
            if(A[i][j] == '.'){
                ...
                int new_val = min_max(0,-500000,500000);
